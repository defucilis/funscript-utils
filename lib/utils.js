...
exports.roundAction = exports.getSpeed = exports.getActionGroups = void 0;
/**
 ...
 */
var roundAction = function (action) {
    var outputAction = {
        at: Math.max(0, Math.round(action.at)),
        pos: Math.max(0, Math.min(100, Math.round(action.pos)))
    };
    if (action.subActions) {
        outputAction.subActions = action.subActions.map(function (subAction) { return exports.roundAction(subAction); });
    }
    if (action.type)
        outputAction.type = action.type;
    return outputAction;
};
exports.roundAction = roundAction;

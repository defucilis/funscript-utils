...
var addFunscriptMetadata = function (funscript) {
    ...
    output.actions = output.actions.sort(function (a, b) { return a.at - b.at; }).map(utils_1.roundAction);
    var duration = output.actions.slice(-1)[0].at;
    var averageSpeed = output.actions.reduce(function (acc, action, index) {
        if (index === 0)
            return acc;
        var speed = utils_1.getSpeed(output.actions[index - 1], output.actions[index]);
        return acc + speed;
    }, 0) / (output.actions.length - 1);
    output.metadata = __assign(__assign({}, output.metadata), { duration: duration, average_speed: averageSpeed });
    return output;
};
...
var convertFunscriptToCsv = function (funscript) {
    ...
    var csv = script.actions.map(function (action) {
        var roundedAction = utils_1.roundAction(action);
